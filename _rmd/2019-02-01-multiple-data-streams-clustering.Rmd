...
Nowadays, **data streams** occur in many scenarios. For example, they are generated from sensors, web traffic, satellites and other interesting use cases.
We have to process them in a fast way and extract from them as much knowledge as we can.
Data streams have theirs own specific characteristics for processing and data mining. For example, they can be very fast, we can not process whole history of streams in a memory, so we have to do it incrementally or in (e.g. sliding) windows.

In this post, I will cover one **data streams clustering** method that was developed by me.
In **R**, you can do data stream clustering by [stream](https://cran.r-project.org/package=stream) package, BUT! there are methods only for one stream clustering (not multiple streams). However, I want to show you clustering of multiple data streams, so from multiple sources (e.g. sensors).

I created clustering method that is adapted for **time series streams** - called [*ClipStream*](https://link.springer.com/article/10.1007/s10618-018-0598-2) [1].
This method is attribute based, so multiple attributes (features) are gathered from multiple streams windows.
You can read more about it from my [journal paper](https://rdcu.be/bbL6y).

I will show you use case on smart meter data - electricity consumption time series (consumers) from London ( [openly available here](https://www.openml.org/d/41060)).
Motivation for clustering such a data is to:
  ...
  4. create more forecastable groups of time series,
  5. monitor changes and behavior.
...
*ClipStream* has following steps (very briefly):
  ...
  2. detect outlier (anomal) streams directly from extracted representations,
...
In this post, I will mainly focus on first 4 parts of my approach, since there are applicable for various task - not only on smart meter data and its forecasting.
Whole method is developed in **unsupervised** fashion (yey!), so **representation**, **clustering** and **outlier detection** of time series streams are "learned" unsupervised.
...
period <- 48 # frequency of time series
```
...
We can see stochastic behavior, but also some pattern.

We can plot, for example, its average daily or weekly profile. We will do it by [TSrepr's package](https://cran.r-project.org/package=TSrepr) function repr_seas_profile.
...
Let's explore whole consumer base, but how when we have more than 1000 time series?
We can cluster time series and just plot its daily patterns for example by created clusters.
We will reduce length of the visualized time series and also number of time series in one plot.
...
cluster them for example by K-means and to 12 clusters:
```{r}
res_clust <- kmeans(data_ave_prof, 12, nstart = 20)
```

Preprocess results with `data.table` package and plot them:
```{r}
...
When we would like to cluster just most recent data (for example 2-3 weeks), and update it regularly every day (or eventought half-hour), automatically detect changes in time series streams and detect anomalies (outlier consumers),
detect automatically number of clusters, and make it as quickly as possible...
...
In *ClipStream* that uses *FeaClip* time series streams representation (see my previous post about [time series represetnations](https://petolau.github.io/TSrepr-time-series-representations/)), a representation can be computed incrementally, clustering are created in batches, outliers are detected straight from representation and etc.

*FeaClip* is interpretable time series representation. It extracts 8 interpretable features from clipped representation (again please see the mentioned blog post). Let's plot result from one day consumption time series:
...
win <- 14
ts_feaclip_long <- repr_windowing(data_cons[1, 1:(period*win)], func = repr_feaclip, win_size = period)
...
gg1 <- ggplot(data.table(Consumption = data_cons[1, 1:(period*win)],
  ...
  geom_vline(xintercept = seq(from=period,to=(period*win)-period, by=period), alpha = 0.75, size = 1.0, col = "dodgerblue2", linetype = 2) +
...
gg2 <- ggplot(data.table(Count = ts_feaclip_long,
  ...
  geom_line(aes(Time, Count)) +
  geom_vline(xintercept = seq(from=8,to=(8*win)-8, by=8), alpha = 0.75, size = 1.0, col = "dodgerblue2", linetype = 2) +
  # geom_point(aes(Time, Count), size = 2, alpha = 0.8) +
...
Now, let's play little bit with `gganimate` and `av` packages and visualize behavior of FeaClip through time (stream).
...
![](/images/post_10/animation.mp4)

We can see how *FeaClip* representation adapts on different behavior of electricity consumption - when it's low around zero, or it gets high etc. - it makes drastic noise reduction and extract only key features from time series.

## Multiple Data Streams

We already got picture of one consumers time series stream.
Let's again play with whole customer base (in our playing scenario with 1000 consumers).
In this part, I would like to make nice animation of outlier detection and clustering phase in one.

#### First iteration

Let's get through the first iteration (window) of streams processing and clustering of *ClipStream*.
So, first thing to do is to subset window of time series streams and compute *FeaClip* representations (window of length 14 days as in the previous case).
```{r}
i = 0
...
The second phase is to detect anomal streams. This is done by "simple" IQR-quantile method from 2 key *FeaClip* features - Sum_1 and number of crossing.
The source code for function `detectOutliersIQR` is in the [*ClipStream* method repo](https://github.com/PetoLau/ClipStream). Let's do it.
```{r}
...
```{r, fig.height=3.8, fig.width=8.5}
ggplot(data.table(Time = 1:ncol(data_win),
                  Value = data_win[which(pc_clip$PC1 == min(pc_clip$PC1)),])) +
  geom_line(aes(Time, Value)) +
  labs(y = "Consumption (kWh)") +
  theme_ts
```

The first scenario is a consumer with the few large moments of high consumption and lot of crossings.
The second scenario is the zero consumption type of a consumer.

The clustering phase is executed by an automatic K-medoids method. The number of clusters are automatically selected by the Davies-Bouldin index from the predefined range of clusters. We will cluster only non-outlier representations, because of reduction of data and also higher stability of clustering results.
```{r}
# n.of.clusters range
k.min <- 8
k.max <- 15
...
Now, flow the data! I will make update every new day during 50 days and plot the results - detected outliers and clusters with also assigned outliers. The code is available on my [GitHub repo](https://github.com/PetoLau/petolau.github.io/tree/master/_Rscripts).

![](/images/post_10/clipstream_1.gif)
...
The results of clustering can be also used for improving forecasting accuracy of aggregated consumption. Read my publications for more information about this topic in the [research section](https://petolau.github.io/research).

### References

[1] Laurinec, P. & Lucká, M.: Interpretable multiple data streams clustering with clipped streams representation for the improvement of electricity consumption forecasting, Data Min Knowl Disc (2018). [DOI link](https://doi.org/10.1007/s10618-018-0598-2)
